...
def test_load_config_reads_the_defaults(tmp_path):
    ...
    assert (config.format, config.cbz, config.metadata) == ("png", True, True)
...
def test_load_config_leaves_the_defaults_alone_when_unset(tmp_path):
    ...
    assert (config.format, config.cbz, config.metadata) == (None, False, False)
