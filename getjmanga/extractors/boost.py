...
The content directory holds a `configuration_pack.json` wrapped as
`{"version": "1.0", "data": <base64>}`. The viewer (`NFBR.a6i.H6f`) unwraps it
with a home-grown cipher keyed by the string `configuration_pack.json`: the
first 96 bytes are three 32-byte keys, the rest is the JSON, and both go
through bit-permutation and RC4 passes before the JSON can be parsed. The
three keys as they come out at the end (`ct`, `st`, `et` in the viewer) also
seed the page file names (`NFBR.a6i.R3s`, when `file-name-version` is set) and
the tile shuffle (`NFBR.b8F`, an xorshift32 PRNG picked from a table of 486
parameter sets). `decode_pack()`, `hashed_page_name()` and `descramble()`
below are ports of those three modules.
"""
...
class Boost(Extractor):
    ...
    def episode(self, url: str) -> Episode:
        ...
        return Episode(
            ...
            series_title=colophon.series_title,
            episode_title=colophon.episode_title,
            pages=tuple(self._pages(pack, content_url)),
            next_url=colophon.next_url,
    ...
    @staticmethod
    def _pages(pack: Pack, content_url: str) -> list[Page]:
